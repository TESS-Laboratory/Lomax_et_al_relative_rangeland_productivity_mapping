...
Calculate and plot pixel-wise correlation between indices and annual precipitation.
...
ppt_cor_hist <- ppt_cor_all %>%
  mutate(method = factor(method, levels = c("rue", "rpi", "restrend_resids"))) %>%
  ggplot(aes(x = ppt_cor, fill = method)) +
  geom_density(position = "identity", alpha = 0.6, colour = "grey30") +
  geom_vline(xintercept = 0) +
  theme_minimal() +
  scale_fill_manual(values = c("#C68039", "#39C680", "#8039C6"),
                    labels = c("RUE", "RESTREND", "RPI")) +
  theme(title = element_text(size = 16),
        axis.title = element_text(size = 20),
        axis.text = element_text(size = 16)) +
...
Plot trends of three time-series methods.
...
restrend_ts_map <- fill +
  create_ts_trend_map(restrend_results[[1]], "PuOr") +
  borders +
  map_layout +
  tm_title("c) RESTREND")
...
rpi_annual_map <- fill +
  ...
  tm_layout(asp = 3/4, frame = FALSE,
            ...
            panel.label.size = 10,
            panel.label.border.color = "white") +
  tm_facets_wrap(nrows = 2, ncols = 2)
