...
For the RPI method, we are interested here only in its ability to explain
...
mae_hist <- ggplot(mae_df, aes(x = mae, fill = method)) +
  ...
  theme_minimal() +
  scale_fill_manual(values = cols4all::c4a("egypt", 3)) +
  ...
  labs(x = expression(MAE~(kg~C~m^-2~y^-1)),
       y = "Density",
       fill = "Method",
       title = "e) Distribution of MAE over study region")
...
rsq_hist <- ggplot(rsq_df, aes(x = rsq, fill = method)) +
  ...
  labs(x = expression(R^2),
       y = "Density",
       fill = "Method",
       title = expression("e) Distribution of "~R^2~"values over study region"))
